...
export const getPartnerFlightKey = (activity: string): Vendor => {
  ...
  return key
    ? AirlinePartner[key as keyof typeof AirlinePartner]
    : Partner.Unknown;
};
...
export const isSas = (transaction: Transaction): boolean => {
  const activity = transaction.activity!;
  return (
    (activity.includes("SK") &&
      transaction.level_points &&
      transaction.level_points > 0) ||
    activity.includesAny(sasActivities)
  );
};

const isAmex = (activity: string): boolean => activity.includes("Amex");
const isAvis = (activity: string): boolean => activity.startsWith("ra ");
const isNorgesgruppen = (activity: string): boolean =>
  activity.includes("NorgesGruppen");
const isRadisson = (activity: string): boolean =>
  activity.includesAny(["Radisson", "Rezidor SAS"]);
const isEuroBonusEarnShop = (activity: string): boolean =>
  activity.includesAny(["EuroBonus Earn Shop", "EuroBonus Shop (NOK)"]);
const isPartnerFlight = (activity: string): boolean =>
  activity.includesAny(Object.keys(AirlinePartner).map((key) => `| ${key}`));
