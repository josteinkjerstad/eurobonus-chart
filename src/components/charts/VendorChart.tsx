...
export const VendorChart = ({ transactions, profiles }: VendorChartProps) => {
  ...
  const vendorOptions = useMemo(() => Array.from(new Set(transactions.map(transaction => transaction.vendor))).sort((a, b) => getDisplayName(a).localeCompare(getDisplayName(b))), [transactions]);

  const groupOptions = (Object.keys(groupedVendors) as GroupVendor[]).filter(group => groupedVendors[group].some(vendor => vendorOptions.includes(vendor))).sort((a, b) => a.localeCompare(b));
  ...
  const filteredVendorPoints = useMemo(() => {
    const filteredTransactions = transactions.filter(
      transaction => selectedVendors.has(transaction.vendor) && selectedYears.has(transaction.year) && Array.from(selectedMembers).some(member => member.id === transaction.profile_id)
    );
    ...
    const groupedTransactions = Object.entries(groupedVendorsState)
      ...
      .map(([group]) => {
        ...
        const points = filteredTransactions.filter(transaction => vendors.includes(transaction.vendor)).reduce((sum, transaction) => sum + transaction.value!, 0);
